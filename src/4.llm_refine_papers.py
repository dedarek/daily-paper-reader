...
from llm import LLMClient, create_llm_client, resolve_max_output_tokens
...
DEFAULT_LLM_PROVIDER = os.getenv("LLM_PROVIDER") or os.getenv("SUMMARY_PROVIDER") or "deepseek"
...
def call_filter(
    ...
    user_prompt = (
        ...
        "4) Reward Enabling Methods only when the method is demonstrated for the requirement's content-safety target; a generic classifier, RAG, edge-inference, parameter-efficient adaptation, or routing method is NOT an enabling match by itself.\n"
        "5) Be strict when the paper's central task is outside content safety/moderation. Generic NLP classification, stance detection, document classification, NER/RAG, position encoding, CVE mapping, privacy/edge inference, and compression papers without a concrete safety target are noise (score 0-2).\n"
        ...
        "7) Do not over-score generic LLM-for-science or infrastructure papers under a composite requirement unless they materially advance the core content-safety task. The paper must be about recognizing, preventing, routing, or evaluating harmful/unsafe content or model safety behavior, not merely use a small model or run on-device.\n\n"
